# Fairmont Apartments — Business Plan

## The Core Thesis: Capture Embedded Upside With Zero Capital Spend

The developer leased Fairmont during Columbus's peak supply cycle, underpricing early rents and offering concessions to fill the building. That mispricing left meaningful loss-to-lease embedded in the rent roll. We enter as the cycle reverses — and collect the upside as leases roll to market through natural turnover.

**No renovation needed. No capital risk. Just execution.**

---

## Three Value Levers

### 1. Loss-to-Lease Capture — ~$290K/Year

- **5.9% LTL** embedded in the current rent roll from lease-up mispricing
- 24 units priced 15%+ below market
- 3BR units have the widest margin: $304/unit below market
- **46% of leases roll by August 2026** — the capture begins immediately at close
- Early lease-up cohort: signed at $1,402–$1,500; current market $2,103+ (50% higher)

**Unit-level strategy:**
- **3BR units:** Push 5–10% on renewal. Currently 5–8% below Westerville avg ($2,270) and 12–15% below New Albany premium tier.
- **1BR/2BR units:** Hold at or slightly below market to protect occupancy during concession burn-off. Selectively push H2 2027 as supply cliff takes hold.
- **AMI units (66 units, 30%):** Built-in 3–5% annual HUD escalator. Near-zero concession exposure, very low turnover. Stabilizing force on the rent roll.

### 2. Concession Burn-Off

- Current concessions being reduced as supply drops and market tightens
- Westerville new deliveries drop 75% in 2026 — landlord pricing power returns
- Columbus absorption running 356% above prior year levels
- As market tightens, concessions evaporate; no action required beyond patience and occupancy discipline

### 3. Other Income Optimization — $191K Lift

- Current other income run rate: ~$259K vs. $606K pro forma target
- **RUBS implementation:** $79–131K/year in utility billing recovery
- **Parking optimization:** Converting surface lots to reserved/premium-priced stalls
- **Pet rent expansion:** Standard program across the community
- Conservative Year 1 target: $400–500K; Year 3 build to $600K+

---

## Rent Growth Model: 3 Verifiable Engines

Our underwriting assumes 4% blended rent growth. That is NOT a market appreciation bet — it's three distinct, verifiable drivers:

1. **AMI Escalator (5.3% CAGR):** 66 affordable units carry a built-in annual HUD rent escalator. These units are fully leased, low-turnover, and insulated from concession pressure. Near-zero volatility.

2. **Loss-to-Lease Capture (~$290K/year):** As early leases expire, in-place rents converge to market. This is arithmetic, not speculation. The gap exists today in the rent roll.

3. **3BR Mark-to-Market (5–8%):** 23 units currently priced below the Westerville average ($2,270) and well below the New Albany premium tier ($2,400+). These push 5–10% on renewal.

**Stress test:** At flat 3% organic growth with a 5.50% exit cap, LP IRR is approximately 14.2%. Under the conservative scenario (3% growth, 6.00% exit cap), LP IRR is approximately 10.5% with a 1.8x equity multiple. The rent growth thesis provides upside, not survival.

---

## Defensive Positioning

Fairmont is built to perform through uncertainty:

| Factor | Our Position |
|--------|-------------|
| Debt Structure | 5.17% fixed, 48-month I/O, Fannie Mae agency — no refinancing risk |
| Renter Income | $102K median HHI (3-mile); 17–18% rent-to-income ratio |
| Basis | $285K/door, 15–20% below replacement cost |
| CapEx | $0 required — brand-new 2025 construction |
| Supply Cliff | Sharp drop in Westerville deliveries through 2026 |
| Self-Management | Gray Capital captures management margin directly |
| Tax Abatement | 15-year, 100% through 2041 — additional cash flow support |

---

## Demand Catalysts (Unmodeled Upside)

Our base case underwriting assumes **existing demand only**. These employers are not modeled:

**Anduril Arsenal-1 — Near-Term Catalyst**
- $1.8B defense manufacturing campus
- 4,008 direct jobs at $132K avg salary
- Ramping operations near Fairmont
- Defense engineers have demonstrated strong renter preference for Class A product

**Intel Ohio One — Longer-Term Catalyst**
- $28B semiconductor campus planned for New Albany
- ~3,000 engineers at $135K+ avg salary
- Timeline subject to Intel's corporate priorities and macroeconomic conditions
- If delivered as planned (2030–31), significant demand catalyst during our hold

These are **demand catalysts layered on top of our underwriting** — not required for the deal to work, but additive if they materialize. The base case assumes existing demand only.

---

## Tax Abatement Compliance

The 15-year, 100% abatement is Fairmont's largest structural advantage. Gray Capital will:
- Implement dedicated CRA compliance tracking from Day 1
- Conduct income certification on all 66 restricted units (80% AMI)
- Engage an Ohio-qualified property manager with abatement compliance experience
- Submit annual reporting 90 days ahead of the March 31 deadline (not at deadline)

The abatement is **transferable with notice** and fully confirmed. The risk is administrative, not economic — and we treat it accordingly.

---

## Gray Capital's Operational Edge

Gray Capital self-manages Fairmont through Gray Residential, retaining full operational control:
- No third-party PM incentive misalignment
- Direct control over concession strategy and lease renewal pricing
- Corporate housing outreach to area employers in advance of close
- Gray Residential coordinating with seller on renewals 90+ days before expiration
- Gray team reaching out to Anduril, Intel, Victoria's Secret, and Bath & Body Works for preferred employer partnerships

---

*Projections are not guaranteed. Past performance does not predict future results. For accredited investors only.*
